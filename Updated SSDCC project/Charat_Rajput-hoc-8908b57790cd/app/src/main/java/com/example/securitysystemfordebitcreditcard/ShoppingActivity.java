    ...
    private void clickButton() {
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (isVailid() == true) {
                    if (entered.length() < 4) {
                        Toast.makeText(ShoppingActivity.this, "Enter correct login pin", Toast.LENGTH_SHORT).show();
                    } else if (current.equals(entered))
                    {
                        Intent intent=new Intent(getApplicationContext(),ChangePassActivity1.class);
                        intent.putExtra("op", "Option 10.");
                        DecimalFormat formatter = new DecimalFormat("#,###,###");
                        long num = Long.parseLong(eTAmount.getText().toString());
                        String yourFormattedString = formatter.format(num);
                        intent.putExtra("msg", "Only one shopping transaction of \u20B9 "+yourFormattedString+"/- "+"will be allowed and card will  automatically lock-up after one time use.");
